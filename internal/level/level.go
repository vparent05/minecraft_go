...
func (b *block) mesh(x, y, z int) []uint32 {
	...
	v12 := uint32((x+1)<<28 | (y+1)<<20 | z<<16 | 0x0<<12 | id)
	v13 := uint32((x+1)<<28 | (y+1)<<20 | (z+1)<<16 | 0x0<<12 | id)

	v21 := uint32((x+1)<<28 | (y+1)<<20 | (z+1)<<16 | 0x0<<12 | id)
	v22 := uint32(x<<28 | (y+1)<<20 | (z+1)<<16 | 0x0<<12 | id)
	v23 := uint32(x<<28 | (y+1)<<20 | z<<16 | 0x0<<12 | id)

	// bottom
	v31 := uint32(x<<28 | y<<20 | z<<16 | 0x1<<12 | id)
	v32 := uint32((x+1)<<28 | y<<20 | (z+1)<<16 | 0x1<<12 | id)
	v33 := uint32((x+1)<<28 | y<<20 | z<<16 | 0x1<<12 | id)

	v41 := uint32((x+1)<<28 | y<<20 | (z+1)<<16 | 0x1<<12 | id)
	v42 := uint32(x<<28 | y<<20 | z<<16 | 0x1<<12 | id)
	v43 := uint32(x<<28 | y<<20 | (z+1)<<16 | 0x1<<12 | id)

	// left
	v51 := uint32(x<<28 | y<<20 | z<<16 | 0x2<<12 | id)
	v52 := uint32(x<<28 | y<<20 | (z+1)<<16 | 0x2<<12 | id)
	v53 := uint32(x<<28 | (y+1)<<20 | (z+1)<<16 | 0x2<<12 | id)
	...
	v72 := uint32((x+1)<<28 | (y+1)<<20 | (z+1)<<16 | 0x3<<12 | id)
	...
	// back
	vB1 := uint32(x<<28 | y<<20 | (z+1)<<16 | 0x5<<12 | id)
	vB2 := uint32((x+1)<<28 | (y+1)<<20 | (z+1)<<16 | 0x5<<12 | id)
	vB3 := uint32((x+1)<<28 | y<<20 | (z+1)<<16 | 0x5<<12 | id)

	vC1 := uint32(x<<28 | y<<20 | (z+1)<<16 | 0x6<<12 | id)
	vC2 := uint32(x<<28 | (y+1)<<20 | (z+1)<<16 | 0x6<<12 | id)
	vC3 := uint32((x+1)<<28 | (y+1)<<20 | (z+1)<<16 | 0x6<<12 | id)
...
func (c *chunk) generateMesh() {
	solidMesh := make([]uint32, len(c.blocks))
